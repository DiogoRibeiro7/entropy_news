...
def _seed_cli_inputs(tmp_path: Path) -> tuple[Path, Path, Path]:
    """Create temporary corpus and embeddings for CLI invocations."""
...
def test_cli_entrypoints_end_to_end(tmp_path: Path) -> None:
    """Exercise the documented CLI workflow via subprocess invocations."""

    pytest.importorskip("torch")
    pytest.importorskip("numpy")

    train_data, new_data, glove_file = _seed_cli_inputs(tmp_path)
    ...
    subprocess.run(forecast_args, check=True)
