...
class NewsDataset(Dataset):
    def __init__(self, encoded_texts: List[List[int]], seq_len: int = 100):
        """Create a dataset of padded token sequences.

        Parameters
        ----------
        encoded_texts : list[list[int]]
            Tokenized texts to be padded.
        seq_len : int, optional
            Maximum length of each sequence, by default ``100``.
        """
        self.seq_len = seq_len
        self.data = [self.pad_sequence(seq) for seq in encoded_texts]

    def pad_sequence(self, seq: List[int]) -> torch.Tensor:
        """Return ``seq`` padded or truncated to ``seq_len + 1`` tokens."""
    ...
    def __getitem__(self, idx: int) -> tuple:
        """Return input and target tensors for the item at ``idx``."""
